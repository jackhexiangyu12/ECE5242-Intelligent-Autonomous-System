...
class Maze():
	...
	def step(self,state,action):
		...
		if cell==self.goal_pos: #remain at exit
			return 0,state,True
		elif (r_next == self.goal_pos[0]) and (c_next == self.goal_pos[1]): # Reach the exit
			v_flag = self.num2flag(state%8)
			return v_flag.count(1), 8*self.cell2idx[(r_next,c_next)] + state%8, True
		else:
			if (r_next,c_next) in Maze.obstacles: # obstacle tuple list
				return 0, state, False
			elif (r_next,c_next)==(cell[0],cell[1]): #outof map
				return 0, state, False
			else:
				v_flag = self.num2flag(state%8)
				if (r_next,c_next) == (0,2):
					...
					v_flag[2] = 1
				return 0, 8*self.cell2idx[(r_next,c_next)] + self.flag2num(v_flag), False
